...
class Aligner:
    ...
    def align_face_images(self, image, align_landmarks, target_landmarks, target_size=256, padding='replicate'):
    ...
    def get_face_tensors_from_frames(self, frames, reference_face, get_all=False, smooth_landmarks=False, smooth_sigma=2, input_image_transforms=None, return_foreground_images=False):
        ...
        for i, original_image in tqdm(enumerate(frames), total=len(frames)):
            ...
            #landmark_rect = self.get_rectangle_from_landmarks(landmark)

            
            landmarks.append(landmark)
            #landmark_rects.append(landmark_rect)
        landmarks = np.stack(landmarks)
        #landmark_rects = np.stack(landmark_rects)
        ...
        for idx, original_image in tqdm(enumerate(frames), total=len(frames)):    
            ...
            #current_image = input_image_transforms(original_image).unsqueeze(0)
            
            #print(f'original image: {np.min(original_image)}-{np.max(original_image)}')
            ...
            #print(f'warped_image image: {np.min(warped_image)}-{np.max(warped_image)}')
            transformed_image = input_image_transforms(warped_image).unsqueeze(0).to(self.device)
            
            # align face landmarks based on reference image and get transformed image
            #transformed_image = self.align_face_tensors(current_image, landmarks[idx], reference_face_landmarks, target_size=reference_face.shape[-2:])

            face_foreground = self.segmenter.get_foreground_BiSeNet(transformed_image, background_classes=[0]).cpu()
